...
class TransformerNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, nhead=4, dropout=0.1, dim_feedforward=2048):
        super(TransformerNetwork, self).__init__()
        # We'll treat each feature dimension as a token.
        # input_size = number of features
        # hidden_size = embedding dim
        # dim_feedforward = dimension of the feedforward network in the Transformer layer
        self.embedding = nn.Linear(1, hidden_size)  # We'll reshape input to (B, input_size, 1) and embed each feature
        self.pos_encoder = PositionalEncoding(hidden_size, dropout=dropout, max_len=input_size)
        ...
        # We'll output a single value, so we can pool the output of the transformer
        self.fc_out = nn.Linear(hidden_size, output_size)

    def forward(self, x):
        # x shape: (batch_size, input_size)
        # Reshape to (batch_size, seq_len, 1) so each feature is a token
        x = x.unsqueeze(-1)  # (B, input_size, 1)
        x = self.embedding(x)  # (B, input_size, hidden_size)
        x = self.pos_encoder(x)  # add positional encoding
        # Pass through the Transformer encoder
        x = self.transformer_encoder(x)  # (B, input_size, hidden_size)
        # Pooling: let's do a simple mean pooling over the sequence dimension
...
# Updated hyperparameters for transformer
patience = 500
nhead = 4
dropout = 0.0
hidden_size = 128
...
for t_span in train_year_spans:
    for e_span in eval_year_spans:
        ...
        for IR in IR_list:
            ...
            log_file = f'log_transformer_{file_name_prefix}.txt'
            
            print_cluster(f'device={device}',log_file)
            ...
            for epoch in range(num_epochs):
                ...
                if test_loss < best_test_loss:
                    ...
                    while did_work==False:
                        try:
                            net_file = os.path.join(neuralNet_folder, f"transformer_netNet_full_trained_{file_name_prefix}.pt")
                            torch.save(model_semnet, net_file)
                            torch.save(model_semnet.state_dict(), net_file.replace("transformer_netNet_full", "transformer_netNet_state"))
                            did_work=True
                        except:
                            time.sleep(0.1)
            ...
            save_loss_file = os.path.join(plot_folder, f"transformer_loss_curve_{file_name_prefix}.png")
            ...
            save_train_auc_file = os.path.join(plot_folder, f"transformer_train_auc_curve_{file_name_prefix}.png")
            ...
            save_test_auc_file = os.path.join(plot_folder, f"transformer_test_auc_curve_{file_name_prefix}.png")
            ...
            if not all_negative_solution_eval: # contain positive cases
                ...
                save_eval_auc_file = os.path.join(plot_folder, f"transformer_eval_auc_curve_{file_name_prefix}.png")

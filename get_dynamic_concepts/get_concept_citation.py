...
while write_file <=len(curr_run_file_paths):
    ...
    vertex_file=os.path.join(vertex_folder, 'concept_part_'+formatted_ID+'.gz')
    log_file_txt=os.path.join(log_folder, 'log_concept_part__'+formatted_ID+'.txt')
    ...
    if not os.path.exists(vertex_file):
        ...
        vertex_lists=[] # define a list to store the edge lists
        ### to avoid conflict writing due to long time running

        with gzip.open(vertex_file, 'wb') as output_file:
            pickle.dump(vertex_lists, output_file)
            ...
            if not lines: # if lines is empty
                ...
                for id_line, line in enumerate(lines):
                    ...
                    for ii in range(len(concepts_for_single_paper)):
                        vertex_lists.append([concepts_for_single_paper[ii],curr_paper_time,curr_all_citations,curr_citations_per_year])
                ...
                # Finish the current file, then store vertex_lists to a pickle file
                with gzip.open(vertex_file, 'wb') as output_file:
                    pickle.dump(vertex_lists, output_file)
                    write_file+=1

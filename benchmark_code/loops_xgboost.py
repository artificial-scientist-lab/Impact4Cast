...
def get_predictions(model, data, solution, eval_batch_size, log_file):
    ...
    for i, batch in enumerate(data_batches, start=1):
        ...
        # For XGBRegressor, we get a continuous output. We interpret it as probability of class 1.
...
for t_span in train_year_spans:
    for e_span in eval_year_spans:
        ...
        for IR in IR_list:
            ...
            log_file = f'log_xgboost_{file_name_prefix}.txt'
            ...
            # Taking a smaller slice for demonstration as in the original code (0:100), keep it the same
            ...
            print_cluster("start training with XGBoost....", log_file)
            ...
            model = xgb.XGBRegressor(
                ...
                verbosity=1,
                eval_metric="rmse",
                early_stopping_rounds=500  # Move this parameter here                
            )

            # Fit the model
            start_time = time.time()
            model.fit(
                input_data_train, train_solution,
                eval_set=[(input_data_test, test_solution)],
                verbose=True
            )
            end_time = time.time()
            print_cluster(f"Training completed in {end_time - start_time:.2f} seconds.", log_file)

            # Compute "loss" as MSE on train/test subsets
            size_of_loss_check = 10000
            train_pred_for_loss = model.predict(input_data_train[:size_of_loss_check])
            train_loss = mean_squared_error(train_solution[:size_of_loss_check], train_pred_for_loss)

            test_pred_for_loss = model.predict(input_data_test[:size_of_loss_check])
            test_loss = mean_squared_error(test_solution[:size_of_loss_check], test_pred_for_loss)

            train_loss_total = [train_loss]
            test_loss_total = [test_loss]

            # Save the trained model (replace tree with xgboost in filenames)
            net_file = os.path.join(neuralNet_folder, f"xgboost_netNet_full_trained_{file_name_prefix}.pkl")
            joblib.dump(model, net_file)

            # Plot loss curve (will be trivial, just one point)
            save_loss_file = os.path.join(plot_folder, f"xgboost_loss_curve_{file_name_prefix}.png")
            ...
            model = joblib.load(net_file)

            # Get predictions for training set
            train_predictions, train_labels = get_predictions(model, input_data_train, train_solution, eval_batch_size, log_file=log_file)
            save_train_auc_file = os.path.join(plot_folder, f"xgboost_train_auc_curve_{file_name_prefix}.png")
            ...
            test_predictions, test_labels = get_predictions(model, input_data_test, test_solution, eval_batch_size, log_file=log_file)
            save_test_auc_file = os.path.join(plot_folder, f"xgboost_test_auc_curve_{file_name_prefix}.png")
            ...
            if not all_negative_solution_eval: # contain positive cases
                eval_predictions, eval_labels = get_predictions(model, eval_feature_dataset, eval_solution_dataset, eval_batch_size, log_file=log_file)
                save_eval_auc_file = os.path.join(plot_folder, f"xgboost_eval_auc_curve_{file_name_prefix}.png")
